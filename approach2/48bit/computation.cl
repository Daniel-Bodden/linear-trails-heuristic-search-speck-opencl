                                ...
                                )
{
    ...
    __private ulong roundResultBackward[23][6];
    __private ulong roundResultForward[23][6];

    //initalize
    for (__private int round = 0; round < 23; round++)
    {
    ...
    for(__private ulong round = 1; round<=nrRounds;round++) // SPECK HAS 23 ROUNDS
    {
        //---------------------------------------------------------------------
        //KEEP INITIAL MASK
        if(round==1)
        ...
        __private uint addedThisRound = 0;;
   ...
   // blocksize = 32;
    ...
    //LOOP OVER ALL POSSIBLE ROUNDS
    for(__private uint round = 1; round <= 23 ;round++) // SPECK HAS 22 ROUNDS
    {
        //KEEP INITIAL MASK  =====================
        if(round==1)
        {
	    ...
	    __private uint addedThisRound = 0;
        __private ulong out_VoutF= 0x0;
        __private ulong out_WoutF= 0x0;
        __private ulong out_biasF = 0;	
	      
		__private ulong VF = inputMaskX ^ inputMaskY;
		
        heuristic_automaton_backward(VF, &xorCheck, &out_VoutF,  &out_WoutF, &out_biasF , wordlength);
			
			
		//CORRECT OUTPUT VALUES
		
        //Xi = V <<< alfa
        maskX =  rotate_left( out_VoutF   ,lshift,wordlength) ;
			
	    //Yi = W  ^(  >>> inputY)
	    maskY =  out_WoutF  ^   rotate_right(inputMaskY  ,rshift,wordlength);
    ...
    // go over all rounds
    for(__private uint round=1; round <= 23; round++)
    {
                                        ...
                                        )
{
    ...
    size_t gidDim2 = get_global_id(1); // work item number
		...
		size_t maxnrthreads = get_global_size(0);	
    
		//nr of of mask created -> upper bound
		/*
	    if ( (gidDim1 > 50000) || (gidDim2 > 50000)    ) 
	    {
	...
	if( (inWords[(gidDim1)] == 0x0) && (inWords[(gidDim2)] == 0x0))
    {
        return;
    }
    else
    {
        ...
        __private ulong rounds = 23;
        __private ulong blocksize = 48;
