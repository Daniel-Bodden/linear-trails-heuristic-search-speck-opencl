                                ...
                                )
{
    ...
    __private ulong roundResultBackward[26][6];
    __private ulong roundResultForward[26][6];

    //initalize
    for (__private int round = 0; round < nrRounds; round++)
    {
   ...
   // blocksize = 32;
    ...
    //LOOP OVER ALL POSSIBLE ROUNDS
    for(__private uint round = 1; round <= 26 ;round++) // SPECK HAS 22 ROUNDS
    {
        //KEEP INITIAL MASK  =====================
        if(round==1)
        {
    ...
    // go over all rounds
    for(__private uint round=1; round <= 26; round++)
    {
                                        ...
                                        )
{
    ...
    size_t gidDim2 = get_global_id(1); // work item number
		...
		/*
	    if ( (gidDim1 > maxnrthreads) || (gidDim2 > maxnrthreads)    ) 
	    {
    ...
    else
    {
        ...
        __private ulong rounds = 26;
        __private ulong blocksize = 64;
